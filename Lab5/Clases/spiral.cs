...
namespace Lab5.Clases
{
    public class spiral
    {
        ...
        public void UncripoAgainstSpiral(string rPath, string wPath, int key)
        {
            using (var file = new FileStream(@wPath, FileMode.OpenOrCreate))
            {
                using (var writer = new BinaryWriter(file))
                {
                    char[,] data = FeelAgainstSpiral(rPath, key);
        ...
        public void UncripSpiral(string rPath, string wPath, int key)
        {
            using (var file = new FileStream(@wPath, FileMode.OpenOrCreate))
            {
                using (var writer = new BinaryWriter(file))
                {
                    char[,] data = FeelSpiral(rPath, key);
                    int rows = data.Length / key;
                    int columns = key;
                    for (int i = 0; i < columns; i++)
                    {
        ...
        private char[,] FeelAgainstSpiral(string rPath, int rows)
        {
            char[,] mander;
            int columns;
            using (var file = new FileStream(@rPath, FileMode.Open))
        ...
        private char[,] FeelSpiral(string rPath, int rows)
        {
            char[,] mander;
            int columns;
            using (var file = new FileStream(@rPath, FileMode.Open))
